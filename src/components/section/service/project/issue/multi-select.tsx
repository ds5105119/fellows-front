    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="w-auto sm:w-40 h-8 justify-between border-0 bg-muted hover:bg-muted focus:bg-muted focus:ring-0 shadow-none"
          disabled={disabled}
        >
      ...
      <PopoverContent className="w-auto sm:w-40 p-0" align="start">
        <Command>
          <CommandGroup>
            {options.map((option) => (
              <CommandItem
