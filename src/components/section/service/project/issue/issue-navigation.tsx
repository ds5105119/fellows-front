...
export default function IssueNavigation() {
  ...
  const tabs = ["이슈", "계약서"] as const;
  const tabMapping: Record<"이슈", ""> & Record<"계약서", "contract"> = { 이슈: "", 계약서: "contract" };

  const handleTabChange = (tab: "이슈" | "계약서") => {
    router.push("/service/project/issue" + tabMapping[tab]);
  };

  const tab = pathname.startsWith("/service/project/issue") ? "이슈" : pathname.startsWith("/service/project") ? "프로젝트" : "";
