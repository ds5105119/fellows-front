...
export function ContractSheet({ contract, session, openSheet, setOpenSheet }: ContractSheetProps) {
          ...
          <div className="p-5 md:p-8 flex flex-col gap-5" ref={targetRef} data-pdf-target>
              ...
              <table className="w-full border-collapse border border-gray-300 text-sm" style={{ tableLayout: "fixed" }}>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="border border-gray-200 px-4 py-1.5" colSpan={6}></td>
                  </tr>
                  <tr>
                    <td className="border border-gray-200 px-4 py-1.5 bg-gray-50 font-semibold text-center" colSpan={2}>
                      개발 비용
            ...
            <div className="flex flex-col space-y-3">
              ...
              <div className="text-xs font-semibold text-muted-foreground">
                ...
                정한 대금에서 공제하고 이를 초과하는 지체상금은 공제되지 아니한다. 다만, 천재지변 기타 이에 준하는 불가항력적인 경우 및 공급자의 책임 없는
                사유로 인하여 지연된 경우에는 지체상금의 일부 또는 전부를 면제할 수 있다.
              </div>
              <div className="text-xs font-semibold text-muted-foreground">
              ...
              <div className="text-xs font-semibold text-muted-foreground">
                ...
                {contract?.custom_name} 와(과) 공급자 Fellows는 붙임에 의하여 위에 대한 계약을 체결하고 신의에 따라 성실히 계약상의 의무를 이행할 것을 확약하며
                본 계약을 증명하기 위해 계약서 2부를 작성 기명 날인하여 {contract?.custom_name} 와(과) 공급자가 각각 1부씩 보관한다.
              </div>
            </div>
                              ...
                              if (isSigned && contract) {
                                ...
                                if (sigCanvas.current?.toDataURL()) {
                                  await updateContracts(contract.name, {
                                    is_signed: true,
                                    custom_customer_sign: sigCanvas.current.toDataURL(),
                                  });
                                }
                                sigCanvas.current?.clear();
                                setIsSigned(false);
