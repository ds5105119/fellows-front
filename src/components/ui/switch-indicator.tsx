...
export function SwitchIndicator({ checked, className }: SwitchIndicatorProps) {
  return (
    <AnimatePresence>
      <motion.div className={clsx("w-9 h-5 rounded-full px-0.5 flex items-center", checked ? "bg-primary" : "bg-neutral-200", className)}>
        <motion.div
          animate={{ marginLeft: checked ? "calc(100% - 1rem)" : "0rem" }}
          transition={{
            type: "spring",
            duration: 0.5,
            bounce: 0.45,
