...
type PreserveAspectRatioAlign = "none" | "xMinYMin" | "xMidYMin" | "xMaxYMin" | "xMinYMid" | "xMidYMid" | "xMaxYMid" | "xMinYMax" | "xMidYMax" | "xMaxYMax";

type PreserveAspectRatioMeetOrSlice = "meet" | "slice";

type PreserveAspectRatio = PreserveAspectRatioAlign | `${Exclude<PreserveAspectRatioAlign, "none">} ${PreserveAspectRatioMeetOrSlice}`;
...
const AnimatedPathText = ({
  ...
  useEffect(() => {
    if (!pathId) {
      setClientId(`animated-path-${Math.random().toString(36).substring(7)}`);
    }
    setIsMounted(true);
  }, [pathId]);

  const id = pathId || clientId || "animated-path-ssr";

  const shouldUseScroll = isMounted && animationType === "scroll" && scrollContainer;

  const safeContainer = shouldUseScroll ? scrollContainer : undefined;
  ...
  }, [scrollYProgress, t, shouldUseScroll]);
  ...
  return (
    <svg className={svgClassName} xmlns="http://www.w3.org/2000/svg" width={width} height={height} viewBox={viewBox} preserveAspectRatio={preserveAspectRatio}>
      ...
      {animationType === "auto" && (
        <text textAnchor={textAnchor} fill="currentColor">
          ...
          >
            {animationType === "auto" && <animate attributeName="startOffset" {...animationProps} from="-100%" to="0%" />}
            {text}
          </textPath>
        </text>
      )}
