...
export const useReportAISummaryStatus = (report_id: string | null | undefined, options: SWRConfiguration<boolean> = {}): SWRResponse<boolean> => {
  const url = `${API_BASE_URL}/estimate/report/${report_id}/status`;
  return useSWR(report_id ? url : null, async (url: string) => await fetcher(url), options);
};
