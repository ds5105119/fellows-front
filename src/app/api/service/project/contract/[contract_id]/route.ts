...
export async function GET(request: Request, { params }: { params: Promise<{ contract_id: string }> }) {
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: "Failed to fetch project data" }, { status: response.status });
    }
